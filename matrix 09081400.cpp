...
//last_edith 18051400
//adding period send status at 18051400

//last_edith 11051400
//kart_relay_051199.cpp
//adding DB for save old state 11051400


//last edith 06071400
//solved bug operator and non stability other


//last Edith 26071400
//solved wifi udp packet loss by send to own ip reciver
//and cancel repeted send status 



//last Edith 09081400
//solved Auto_operator  set by new system control
//and cancel repeted send status 
...
int read_pin_bussy=1; //1:access free   0:access denide
int Delay_B_A=1;//delay befor Access to bus
...
#define Is_on_or_off 2
 ...
 struct sockaddr_in si_me, si_other;
    ...
    int usual_Relay[16][2][8];
    int usual_Relay_delay[16][2][8];
  // int usual_Relay_change[16][2][3];//  3: [1:status change  2:now_time  3:exe_time]
   int Execuite_status[30][2][23];//1:order_number  2:[ 0:status_change  1:kart number    2:last_first-->first 3:relay_number 4-11:value   12:now_time  13:exe_time]
...
void pin_change(int kart_number,int control_latch,int first_last)
{
 ...
 void  Read_Old_Status_Usual_from_db()
{
   ////////////////DB variable///////////////////////////
      sql::Driver *driver;
      sql::Connection *con;
      ...
      try {
          ...
          con = driver->connect("127.0.0.1", "root", "bakhman33QE");
  
  
          /* Connect to the MySQL test database */
          con->setSchema("Line_Control_Monitor");
    ...
    try {
            ...
            std::string str ("SELECT * From Usual_Relay");
	   ...
	   // int i=1;
	    ...
	    int value=-2;
	     while (res->next()) 
	     {
	       Kart_number=res->getInt("kart_number");
	       first_last=res->getInt("first_last");
	       Relay_number=res->getInt("Relay_number");
	       value=res->getInt("value");
	      // printf("\n value=%d",value);
		 usual_Relay[Kart_number][first_last][Relay_number]=value;
	      }
...
int first_last=0,Relay_number=0;
 ...
 void  Read_Old_Status_Chanel_from_db()
{
   ////////////////DB variable///////////////////////////
      sql::Driver *driver;
      sql::Connection *con;
    ...
    try {
            ...
            std::string str ("SELECT * From Chanely_Relay");
            res=stmt->executeQuery(str);
	   ...
	   // int i=1;
	    ...
	    int value=-2;
	     while (res->next()) 
	     {
		 chanel_number=res->getInt("id");
		 value=res->getInt("value");
		 chanely_Relay[chanel_number]=value;
	      }
...
void insert_data_usually_relay(int kart_number,int first_last,int Relay_number ,int value)
  {
 ...
 std::string str_id("");
            ...
            std::string str_kart_number(to_string(kart_number));
            str_id.insert (str_id.size(),str_kart_number);
	    
	    std::string str_first_last(to_string(first_last));
            str_id.insert (str_id.size(),str_first_last);

	    std::string str_Relay_number(to_string(Relay_number));
            str_id.insert (str_id.size(),str_Relay_number);


//INSERT INTO Usual_Relay (id, kart_number,first_last,Relay_number,value ) VALUES(1,0, 0,1,1) 
//

            std::string str ("INSERT INTO `Usual_Relay`(`id`,`kart_number`, `first_last`, `Relay_number`, `value`) VALUES ('");
         
	    
            str.insert (str.size(),str_id);
	     std::string str11 ("','");
            str.insert (str.size(),str11);
	    
	  
            str.insert (str.size(),str_kart_number);
	    
	    std::string str3 ("','");
            str.insert (str.size(),str3);
	    
	 
            str.insert (str.size(),str_first_last);
	    ...
	    std::string str5 ("','");
            ...
            str.insert (str.size(),str_Relay_number);
	    
	    std::string str7 ("','");
            str.insert (str.size(),str7);
	    
	    
	    std::string str_value(to_string(value));
            str.insert (str.size(),str_value);
	    
	    std::string str9 ("')");
            str.insert (str.size(),str9);
  
            std::string Replace (" ON DUPLICATE KEY UPDATE value=");
            str.insert (str.size(),Replace);
...
void  Write_Old_Status_Usual_to_db()
{
    int i,j,k=0;
    for(i=0;i<16;i++)
    {
	 ...
	 for(k=0;k<=7;k++)
	  {
	       insert_data_usually_relay(i,j,k,usual_Relay[i][j][k]) ;
	  }
      }
...
void insert_data_Chanelly_relay(int chanel_number,int value)
  {
    ...
    try {
	    ...
	    std::string str_id(to_string(chanel_number));  
            std::string str ("INSERT INTO `Chanely_Relay`(`id`,`value`) VALUES ('");
...
void insert_data_relation_list_Rasi_0(int input_state,int output_state,int value)
  {
    ...
    try {
	    ...
	    std::string str_input_state(to_string(input_state));  
	    std::string str_output_state(to_string(output_state)); 
	    std::string str_value(to_string(value)); 
	    std::string str_space ("','");
	    
	    std::string str_id("");  
	    str_id.insert (str_id.size(),str_input_state); 
	    str_id.insert (str_id.size(),str_output_state); 
            std::string str ("INSERT INTO `Relation_list_Raise_event_0`(`id`,`input_state`,`output_state`,`value`) VALUES ('");
         
	    str.insert (str.size(),str_id);     str.insert (str.size(),str_space);
            str.insert (str.size(),str_input_state);     str.insert (str.size(),str_space);
	    str.insert (str.size(),str_output_state);    str.insert (str.size(),str_space);
            str.insert (str.size(),str_value);
	    ...
	    std::string Replace2 (",input_state=");
	    str.insert (str.size(),Replace2);
	     str.insert (str.size(),str_input_state);
	     
	    std::string Replace3 (",output_state=");
	    str.insert (str.size(),Replace3);
	     str.insert (str.size(),str_output_state);
 //printf("\n Follow this command: %s", str.c_str());
...
void insert_data_relation_list_Rasi_1(int input_state,int output_state,int value)
  {
    ...
    try {
	    ...
	    str_id.insert (str_id.size(),str_input_state); 
	    str_id.insert (str_id.size(),str_output_state); 
            std::string str ("INSERT INTO `Relation_list_Raise_event_1`(`id`,`input_state`,`output_state`,`value`) VALUES ('");
...
void write_relation_All_list_rase_1__0_to_255_to_db()
  {
    ...
    try {
            ...
            std::string str ("UPDATE `Relation_list_Raise_event_0` SET `value`=255");
           stmt->execute(str);
	    
	    
	     std::string str2 ("UPDATE `Relation_list_Raise_event_1` SET `value`=255");
            stmt->execute(str2);
...
void zzzzzzzzz(int number_kart,int F_L_k,int value)
{
    wait_for_repository_B(); 
    Execuit_repository_B();
    
     Execuite_status[number_kart][F_L_k][0]=value;
    free_repository_B();
}


void On_OF_set_to_Execuite_status(int number,int on_off,int exe_time)
{
 // printf("\n\n\n %d=%d",number,on_off);
    int first_last=0;
...
if(exe_time==0)
{
  F_number=kart_number;   
}
else
{
  F_number=kart_number+16;
}
   
  //printf("\n F_number=%d",F_number);
// printf("\n");

  /*printf("\n  Execuite_status[0][0]=%d", Execuite_status[0][0]);
  printf("\n  Execuite_status[1][0]=%d", Execuite_status[1][0]);
  printf("\n  Execuite_status[2][0]=%d", Execuite_status[2][0]);
  printf("\n  Execuite_status[3][0]=%d", Execuite_status[3][0]);
  printf("\n  Execuite_status[4][0]=%d", Execuite_status[4][0]);
  printf("\n  Execuite_status[5][0]=%d", Execuite_status[5][0]);*/


  /*printf("\nusual_Relay[%d][%d]=%d %d %d %d %d %d %d %d ",kart_number,first_last
		     ,usual_Relay[kart_number][first_last][0]
		     ,usual_Relay[kart_number][first_last][1]
		     ,usual_Relay[kart_number][first_last][2]
		     ,usual_Relay[kart_number][first_last][3]
		     ,usual_Relay[kart_number][first_last][4]
		     ,usual_Relay[kart_number][first_last][5]
		     ,usual_Relay[kart_number][first_last][6]
		     ,usual_Relay[kart_number][first_last][7]);*/
 ...
 long int now_time=time_stamp_now();
 int exe_time_calc=(int)(now_time+exe_time);
     
  //  printf("\n  now_time=%ld", now_time);
  //   printf("\n  exe_time=%d", exe_time);
   // printf("\n  exe_time_calc=%d", exe_time_calc);
     
   // 2:[ 0:status_change  1:kart number    2:last_first 3:relay_number 4:value  5:now_time  6:exe_time ]
   int h=0;
    ...
    if(Execuite_status[F_number][first_last][0]==1)
    {
	 Execuite_status[F_number][first_last][4+relay_number]=on_off;
    }
    else
    {
	for(h=0;h<=7;h++)
	 {
	      Execuite_status[F_number][first_last][4+h]=255;
	 }
	Execuite_status[F_number][first_last][4+relay_number]=on_off;
    }
 ...
 /*  for(h=0;h<=7;h++)
	 {
	      printf("\nExecuite_status[%d][%d]=%d",F_number,4+h,Execuite_status[F_number][4+h]);
	 }
...
//1:order_number  2:[ 0:status_change  1:kart number    2:last_first-->last 3:relay_number 4-11:value  12-19:value2 20:now_time  21:exe_time 22:first_last-->Last]
   
     Execuite_status[F_number][first_last][1]=kart_number;

     Execuite_status[F_number][first_last][2]=first_last;
     //Execuite_status[F_number][first_last][22]=first_last;
  
     Execuite_status[F_number][first_last][3]=relay_number;
    
     Execuite_status[F_number][first_last][12]=now_time;
     Execuite_status[F_number][first_last][13]=exe_time_calc;
     zzzzzzzzz(F_number,first_last,1);
      insert_data_usually_relay(kart_number,first_last,relay_number,on_off);
    
  printf("\n F_number=%d  Execuite_status[%d][13]=%d",F_number,F_number,Execuite_status[F_number][first_last][13]);
...
void On_OF_set_chanelly_to_usual(int number,int on_off)
{
 ...
 // printf("\n\n kart_number=%d",kart_number+1);
   ...
   }else
   {
    ...
    usual_Relay[kart_number][first_last][relay_number]=on_off; 
   // usual_Relay_change[kart_number][first_last]=1;
}
...
void Sena_Data_to_Ethe_all()
{
	//now reply the client with the same data
       
       //SEND_bufer[0]=0;
      // SEND_bufer[1]=234;
 //printf("\n Sena_Data_to_Ethe_all");
        if (sendto(sR,Send_buf,34, 0,(struct sockaddr*) &si_meR, slen) == -1)
        {
            perror("sendto()");
        }
}

void Sena_Data_to_Ethe_all_to_last_send_packet()
{
	//now reply the client with the same data
...
// printf("\n Sena_Data_to_Ethe_all_to_last_send_packet");
        if (sendto(sR,Send_buf,34, 0,(struct sockaddr*) &si_other, slen) == -1)
        {
            perror("sendto()");
        }
}
...
void Sena_Data_to_Ethe()
{
 ...
 //printf("\n >>>>>> %d %d %d %d %d",Send_buf[0],Send_buf[1],Send_buf[2],Send_buf[3],Send_buf[4]); 
...
void Packet_send_all_to_last_send_packet()
{
...
void operate_relation_list(int curent_sts,int number_change)
{
 ...
 //number_change=number_change-1;
  ...
  if(curent_sts==0)
  {
    for(i=0;i<256;i++)
    {
        printf("\n Relation_list_0__[%d][%d]=%d",number_change,i,Relation_list_0__[number_change][i]);
...
void check_data()
{
	...
	//printf("\n Recive_packet %x %x %x %x",Recive_buf[0],Recive_buf[1],Recive_buf[2],Recive_buf[3]);
	if(Recive_buf[0]==1)//click Req   |  ID  |-------| 0=off 1=on |
	{
		...
		if(Contactor_mode==0)
		{
		 //  printf("\n inja   0");
		    ...
		    //access_in_kart(true);
			    ...
			    insert_data_Chanelly_relay((Chanel_ID-1)/2,-1);
			    On_OF_set_to_Execuite_status((Chanel_ID-1),On_Or_Off,0);
			    //On_OF_set_chanelly_to_usual((Chanel_ID-1),On_Or_Off);
		...
		else if(Contactor_mode==1)
		{
			     ...
			     if((On_Or_Off==1)||(On_Or_Off==0))
			    {
			        ...
			        //Write_Old_Status_Chanel();
				insert_data_Chanelly_relay(Chanel_ID-1,On_Or_Off);
				//int i=0;
				 ...
				 // On_OF_set_chanelly_to_usual(((Chanel_ID-1)*2)  ,1);
				    ...
				    if(On_Or_Off==1)
				    {
					 ...
					 // on_off(((Chanel_ID-1)*2),0);
					    ...
					    On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)+1,0,0);
					    On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)  ,1,0);
					    On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)  ,0,500);
				    }
				    
				    if(On_Or_Off==0)
				    {
				      
				            On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)+1,1,0);
					    On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)  ,0,0);
					    On_OF_set_to_Execuite_status(((Chanel_ID-1)*2)+1  ,0,500);
...
{
  printf("\n Recive_packet %x %d %d %d",Recive_buf[0],Recive_buf[1],Recive_buf[2],Recive_buf[3]);
	    int input_nr=(Recive_buf[1]);  
	    int Raise_event=Recive_buf[2];    
	    int outPut_nr=(Recive_buf[3]); 
	    int operate=Recive_buf[4];
	    printf("\n input_nr=%d   outPut_nr=%d",input_nr,outPut_nr);
	    if(Raise_event==0)
	    {
	      Relation_list_0__[input_nr][outPut_nr]=operate;
	    //  write_relation_list_0_();
	      insert_data_relation_list_Rasi_0(input_nr,outPut_nr,operate);
	    }
	    else if(Raise_event==1)
	    {
	      Relation_list_[input_nr][outPut_nr]=operate;
	     // write_relation_list_1_();
	       insert_data_relation_list_Rasi_1(input_nr,outPut_nr,operate);
	    }	    
}
...
else if(Recive_buf[0]==0xaa)//request all status  
{
  int i=0;
  int this_status=0;
  for(i=0;i<=255;i++)
  {
	 ...
	 this_status=0;
         ...
         this_status=(this_status)+(Status_Table[i]<<(i%8));
   //   printf("\n Status_Table[%d]=%d  Status_Table[i]<<%d=%d   this_status=%d",i,Status_Table[i],(i%8),(Status_Table[i]<<(i%8)),this_status);
       
     
     if((i%8)==7)
      {
       Send_buf[(i/8)+2]=this_status;
     // printf("\n Send_buf[%d]=%d",(i/8)+2,Send_buf[(i/8)+2]);
     }
     //printf("\n i=%d  i/8=%d  i/8=%d",i,i%8,i/8);
   } 
  // printf("\n injaa");
    Packet_send_all_to_last_send_packet();
...
void *Ethernet_get_check(void *x)
{
	while(1)
	{	
...
void  read_relation_list_Raise_event_0_from_db()
{
   ////////////////DB variable///////////////////////////
      sql::Driver *driver;
      sql::Connection *con;
	...
	for (j=0;j<256;j++)
	  {
	      for (i=0;i<256;i++)
		{
		   Relation_list_0__[j][i]=255;  
		}
	  }
	  
            std::string str ("SELECT * From Relation_list_Raise_event_0");
            res=stmt->executeQuery(str);
	   ...
	   // int i=1;
	    ...
	    int value=-2;
	     while (res->next()) 
	     {
	       input_state=res->getInt("input_state");
	       output_state=res->getInt("output_state");
	       value=res->getInt("value");
		 Relation_list_0__[input_state][output_state]=value;
	      }
...
void  read_relation_list_Raise_event_1_from_db()
{
   ////////////////DB variable///////////////////////////
      sql::Driver *driver;
      sql::Connection *con;
	...
	for (j=0;j<256;j++)
	  {
	      for (i=0;i<256;i++)
		{
		   Relation_list_[j][i]=255;  
		}
	  }
            std::string str ("SELECT * From Relation_list_Raise_event_1");
            res=stmt->executeQuery(str);
	   ...
	   // int i=1;
	    ...
	    int value=-2;
	     while (res->next()) 
	     {
	       ...
	       output_state=res->getInt("output_state");
	       value=res->getInt("value");
		 Relation_list_[input_state][output_state]=value;
		 printf("\n Relation_list_[%d][%d]%d",input_state,output_state,Relation_list_[input_state][output_state]);
	      }
...
//insert_data_usually_relay(0,0,0,1);
...
read_relation_list_Raise_event_0_from_db();
 ...
 ether_init();
  ...
  init_board();
   ...
   restor_old_stutus();
		  ...
		  int thread_args[5];
		  thread_args[1] = 444;
		  pthread_create(&threads[1], NULL, Ethernet_get_check, (void *) &thread_args[1]);
    ...
    for(i=0;i<16;i=i+1)
	  {	
		    Execuite_status[i][0][0]=-1;  
		    Execuite_status[i][1][0]=-1;     
	  }
 ...
 // int rel_nb=0;
  ...
  int h=0;
  int status_send_period_number=0;  
  while(1)
	{
       ...
       for(i=0;i<=15;i++)
	{
	  ...
	  Read_pin_kart(i); //read feedback line kart by kart
	  int j=0;
	  int pin_number=0;
	 ...
	 for(j=0;j<=15;j++)
	  ...
	  //  {
	   ...
	   // }
		...
		// printf("\n curent_status=%d pin_number=%d \n",curent_status,pin_number);
		
	       }
	  }
	 }
	 
	 if(status_send_period_number>=1)
	 {
	    status_send_period_number=0;
	   // status_send_period();  
	  }
	  else
	  {
	    status_send_period_number++;
	  }
	    ...
	    for(i=0;i<30;i++)
	  {	
	       for(F_L_k=0;F_L_k<2;F_L_k++)
		{
	  		
		  if(Execuite_status[i][F_L_k][0]==1)
		  {		  
		    now_time=time_stamp_now();
		    //printf("\n Execuite_time=%d",Execuite_status[i][6]);
		   // printf("\n now_time=%ld",time_stamp_now());
		    
		     if(now_time>=Execuite_status[i][F_L_k][13])
		     {
		       
	      // 2:[ 0:status_change  1:kart number    2:last_first 3:relay_number 4:value  12:now_time  13:exe_time ]
  
		        K_n=Execuite_status[i][F_L_k][1];
		        //F_L_k=Execuite_status[i][2];
			//value=Execuite_status[i][4];
...
//printf("\n value=%d",value);
  // printf("\n value=%d",((value)&(128))>>7);
		      for(h=0;h<8;h++)
		      {
			if(Execuite_status[i][F_L_k][4+h]!=255)
			{
			  printf("\n         Execuite_status[%d][%d]=%d",i,4+h,Execuite_status[i][F_L_k][4+h]);
			    usual_Relay[K_n][F_L_k][h]=Execuite_status[i][F_L_k][4+h];
			}
		      }
		      
		     
 
		      printf("\n         usual_Relay[%d][%d]=%d %d %d %d %d %d %d %d ",K_n,F_L_k
		     ,usual_Relay[K_n][F_L_k][0]
		     ,usual_Relay[K_n][F_L_k][1]
		     ,usual_Relay[K_n][F_L_k][2]
		     ,usual_Relay[K_n][F_L_k][3]
		     ,usual_Relay[K_n][F_L_k][4]
		     ,usual_Relay[K_n][F_L_k][5]
		     ,usual_Relay[K_n][F_L_k][6]
		     ,usual_Relay[K_n][F_L_k][7]);
			...
			if(F_L_k==1)
			{
			     pin_change_old(K_n,control_d_W_last,1);
			}
			else 
			{ 	
			   pin_change_old(K_n,control_A_W_first,0);
			}
			Execuite_status[K_n+16][F_L_k][13]=now_time+1;
			zzzzzzzzz(i,F_L_k,-1);
			//Execuite_status[i][0]=-1;
		    }
	  ...
	  {	
	    ...
	    if(operator_list[i]==1)
	    {
	      printf("\n *********operator_list[%d]=%d",i,operator_list[i]);
	         chanely_Relay[i-1]=1;
		// Write_Old_Status_Chanel();
		insert_data_Chanelly_relay(i-1,1);
		
		//on_off(((i-1)*2)+1,0);
		//on_off(((i-1)*2)  ,1);
		//delay(500);
		//on_off(((i-1)*2),0);
		//On_OF_set_chanelly_to_usual(((i-1)*2)+1,0);
		//On_OF_set_chanelly_to_usual(((i-1)*2)  ,1);
				
				
		On_OF_set_to_Execuite_status(((i-1)*2)+1,0,0);
	        On_OF_set_to_Execuite_status(((i-1)*2)  ,1,0);
	        On_OF_set_to_Execuite_status(((i-1)*2)  ,0,500);
	     ...
	     else if(operator_list[i]==0)
	    {
	        ...
	        chanely_Relay[i-1]=0;
		 //Write_Old_Status_Chanel();
		 insert_data_Chanelly_relay(i-1,0);
	      
	      //  on_off(((i-1)*2)  ,0);
		//on_off(((i-1)*2)+1,1);
              //  delay(500);
		//on_off(((i-1)*2)+1,0);
		//On_OF_set_chanelly_to_usual(((i-1)*2)  ,0);
               // On_OF_set_chanelly_to_usual(((i-1)*2)+1,1);

		 On_OF_set_to_Execuite_status(((i-1)*2)+1,1,0);
		 On_OF_set_to_Execuite_status(((i-1)*2)  ,0,0);
		 On_OF_set_to_Execuite_status(((i-1)*2)+1  ,0,500);
